...
// v6 — LinkedIn (30 derniers jours) : #cleareconDL | "CleaRecon DL" | "CleaReconDL"
// Priorité : 1) Bing Web Search API (freshness=Month)  2) SerpAPI (Google, tbs=qdr:m)
//            3) Google HTML via r.jina.ai (tbs=qdr:m)  4) Yahoo HTML via r.jina.ai (age=1m)
// Écrit des .md dans src/content/li-clearecondl, avec vignette si possible.
...
function firstAbsolute(u, base) {
  try { return new URL(u, base).href; } catch { return ""; }
}
function extractLinkedInImage(html, base) {
  // 1) og:image / twitter:image
  const og = pick(/<meta[^>]+property=["']og:image["'][^>]*content=["']([^"']+)["']/i, html);
  const tw = pick(/<meta[^>]+name=["']twitter:image(?::src)?["'][^>]*content=["']([^"']+)["']/i, html);
  const candidates = [og, tw].map(u => firstAbsolute(u, base)).filter(Boolean);
  if (candidates.length) return candidates[0];

  // 2) liens media.licdn.com dans le HTML
...
function writeItem(link, meta){
  ensureDir(OUT_DIR);
  const d=new Date(); // date d'indexation (la recherche already = 30j)
...
function YAHOO_Q(q, b=1){
  // fr2=time & age=1m pour "dernier mois"
...
async function main(){
  ...
  if(!links || !links.length){
    console.log("[LI] Aucun lien ce run (ou throttling). On réessaiera au prochain run.");
    return;
  }
