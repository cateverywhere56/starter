...
// LinkedIn posts contenant "#cleareconDL" / "CleaRecon DL" / "CleaReconDL"
// - Source: SERP Bing (pages 1..10) via r.jina.ai (pas d'accès direct à linkedin.com)
// - Garde uniquement les vrais posts: /posts/... ou /feed/update/urn:li:activity:...
// - Récupère title/description/image via OG (proxy), écrit .md dans src/content/li-clearecondl
...
const BING = (q, first=1) => `https://www.bing.com/search?q=${encodeURIComponent(q)}&setlang=fr&first=${first}`;
...
async function fetchOgMetaLinkedIn(url) {
  // Toujours via proxy pour éviter le 999
  ...
  for (const q of BING_QUERIES) {
    for (let first=1; first<=91; first+=10) {
      ...
      try {
        ...
        if (!res.ok) { console.error(`[LI] SERP ${res.status} ${first}`); continue; }
...
function writeItem(link, meta) {
  ensureDir(OUT_DIR);
  const d = new Date(); // pas d'horodatage public accessible → on timestamp à l’indexation
...
async function main() {
  ...
  if (!rawLinks.length) { console.log("[LI] Aucun lien trouvé en SERP."); return; }

  // dé-dup et filtre final par mots-clés sur le titre OG si possible
  const seen = new Set();
  let created = 0;
  ...
  for (const link of rawLinks) {
    ...
    // Filtre: doit contenir #clearecondl / clearecon dl / clearecondl (insensible casse)
    const text = `${meta.title} ${meta.desc}`.toLowerCase();
